...
public final class MusicUtils {
	...
	public static final long[] getSongListForArtist(final Context context, final String id) {
		...
		Cursor cursor = context.getContentResolver().query(
				...
				null, AudioColumns.ALBUM_KEY + "," + AudioColumns.TRACK);
		if (cursor != null) {
	...
	public static final long[] getSongListForAlbum(final Context context, final String id) {
		...
		Cursor cursor = context.getContentResolver().query(
				MediaStore.Audio.Media.EXTERNAL_CONTENT_URI, projection, selection.toString(),
				null, AudioColumns.TRACK + ", " + MediaStore.Audio.Media.DEFAULT_SORT_ORDER);
		if (cursor != null) {
			final long[] mList = getSongListForCursor(cursor);
			cursor.close();
			cursor = null;
	...
	public static final long getIdForPlaylist(final Context context, final String name) {
		...
		if (cursor != null) {
			if (cursor.moveToFirst()) {
				do {
					id = cursor.getInt(0);
					if (getSongListForPlaylist(context, Long.toString(id)).length > 0) {
						break;
					}
	...
	public static final long getIdForArtist(final Context context, final String name) {
		...
		if (cursor != null) {
			if (cursor.moveToFirst()) {
				do {
					id = cursor.getInt(0);
					if (getSongListForArtist(context, Long.toString(id)).length > 0) {
						break;
					}
				} while (cursor.moveToNext());
			}
	...
	public static final long getIdForAlbum(final Context context, final String name) {
		Cursor cursor = context.getContentResolver().query(
				...
				AlbumColumns.ALBUM + "=?", new String[] { name }, AlbumColumns.ALBUM);
		int id = -1;
		if (cursor != null) {
			if (cursor.moveToFirst()) {
				do {
					id = cursor.getInt(0);
					if (getSongListForAlbum(context, Long.toString(id)).length > 0) {
						break;
					}
				} while (cursor.moveToNext());
			}
	...
	public static final String getAlbumArtist(final Context context, final String name) {
		Cursor cursor = context.getContentResolver().query(
				MediaStore.Audio.Albums.EXTERNAL_CONTENT_URI, new String[] { AlbumColumns.ARTIST },
				AlbumColumns.ALBUM + "=?", new String[] { name }, AlbumColumns.ALBUM);
	...
	public static void setRingtone(final Context context, final long id) {
		...
		final String[] projection = new String[] { BaseColumns._ID, MediaColumns.DATA,
				MediaColumns.TITLE };
	...
	public static final String getSongCountForAlbum(final Context context, final String name) {
		...
		Cursor cursor = context.getContentResolver().query(
				...
				new String[] { AlbumColumns.NUMBER_OF_SONGS }, AlbumColumns.ALBUM + "=?",
				new String[] { name }, AlbumColumns.ALBUM);
	...
	public static final String getReleaseDateForAlbum(final Context context, final String name) {
		...
		Cursor cursor = context.getContentResolver().query(
				MediaStore.Audio.Albums.EXTERNAL_CONTENT_URI,
				new String[] { AlbumColumns.FIRST_YEAR }, AlbumColumns.ALBUM + "=?",
				new String[] { name }, AlbumColumns.ALBUM);
