...
public final class MusicUtils {
	...
	public static final int getSongCountForAlbum(final Context context, final long id) {
		...
		// group by section
		selection.append(" ) GROUP BY ( " + AudioColumns.ALBUM_ID);
		Cursor cursor = context.getContentResolver().query(
				MediaStore.Audio.Media.EXTERNAL_CONTENT_URI, new String[] {
				/* 0 */
				"COUNT(" + BaseColumns._ID + ")" }, selection.toString(), null, null);
	 ...
	 * @param id
	 *            The id of the artist.
	 * @return The song count for an artist.
	 */
	public static final int getSongCountForArtist(final Context context, final long id) {
		if (id == -1) {
			return 0;
		}
		final StringBuilder selection = new StringBuilder();
		...
		// group by section
		selection.append(" ) GROUP BY ( " + AudioColumns.ARTIST_ID);
		Cursor cursor = context.getContentResolver().query(
				MediaStore.Audio.Media.EXTERNAL_CONTENT_URI, new String[] {
				/* 0 */
				"COUNT(" + BaseColumns._ID + ")" }, selection.toString(), null, null);
	 ...
	 * @param id
	 *            The id of the artist.
	 * @return The album count for an artist.
	 */
	public static final int getAlbumCountForArtist(final Context context, final long id) {
		if (id == -1) {
			return 0;
		}
		final StringBuilder selection = new StringBuilder();
		...
		Cursor cursor = context.getContentResolver().query(
				MediaStore.Audio.Media.EXTERNAL_CONTENT_URI, new String[] {
				/* 0 */
				"1" }, selection.toString(), null, null);
