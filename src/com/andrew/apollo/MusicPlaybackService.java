...
public class MusicPlaybackService extends Service {
	 ...
	 */
	private static final String[] ALBUM_PROJECTION = new String[] { MediaStore.Audio.Albums.ALBUM,
			MediaStore.Audio.Albums.ARTIST, MediaStore.Audio.Albums.LAST_YEAR };
	...
	private void updateNotification() {
		if (!mAnyActivityInForeground && isPlaying()) {
			mNotificationHelper.buildNotification(getAlbumName(), getArtistName(), getTrackName(),
					getAlbumId(), getAlbumArt(), isPlaying());
	...
	private Cursor openCursorAndGoToFirst(Uri uri, String[] projection, String selection,
	...
	private void reloadQueue() {
		...
		if (qlen > 1) {
			...
			if (D) {
				Log.d(TAG, "restored queue, currently at position " + position() + "/" + duration()
						+ " (requested " + seekpos + ")");
			}
	...
	public Bitmap getAlbumArt() {
		...
		final Bitmap bitmap = mImageFetcher.getArtwork(getAlbumName(), getAlbumId(),
				getArtistName());
		return bitmap;
	}
